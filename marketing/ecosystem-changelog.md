...
**Nine Tuesday names rendered in the tile** (`tuesday-names-preview.html`) rather than left as a list, because a name reads differently at 50px than in a table row. **Seeing them at size changed two judgements:** `Street Taco Tuesday` is long enough to weaken the headline and push to two lines on narrow screens; `One More Tuesday` sets beautifully and is the same character count as the current name, so it drops in with no layout change; `Second Taco's Free` confirmed as a good SMS line but wrong as a tile headline.

**🔴 A legibility failure I fixed the wrong way twice.** The line-art watermark was smothering the prices — BOGO/$6/$30 unreadable. I responded by **tuning opacity, twice**, before Ramsey pointed out it was still illegible. **He was right and my approach was wrong:** dense line art still breaks type at 5% opacity, and below that it isn't worth having.
**Fixed structurally** (`no-overlap.html`) with three options that have **zero art/text overlap**: **H** no art (papel picado + ticket border carry it) · **I** art in its own navy panel · **J** art in a band across the top.
**Recommended I** — it's the only option where fixing the problem *gains* something: the art goes from an unreadable 5% smudge to a **full-strength graphic**, because it sits on navy instead of behind type. It's more visible than the watermark ever was.
**Rule recorded in the design skill:** *art never goes behind text — give it its own zone or leave it out; never solve an overlap with opacity.*
