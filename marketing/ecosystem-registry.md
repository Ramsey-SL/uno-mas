...
19. **31 live-site assets violate the naming convention** — ⚠️ **Recommended resolution changed 2026-08-25: TAG, don't rename.** Renaming changes delivery URLs and the live site references them by public_id, so a rename needs a coordinated redeploy for cosmetic gain. Tagging achieves the actual goal (findability) with zero breakage. See `marketing/cloudinary-organization-plan.md`. *(original note follows)* (`IMG_0245`, `2R7A8526`, `carne-asada-knife-hero`, `uno-mas/website/cantina/d72ajps8zp3jflmqynne`, …). They work but are unsearchable by `YYYYMMDD_UM_CATEGORY_Subject_v#`. Worth a rename pass — Cloudinary renames are non-breaking if done via `asset_folder`/rename API with URL preservation, but the site references them by public_id, so **the site must be updated in the same change.**
...
8. ~~**Thursday promo changed 2026-08**~~ *(historical record, RESOLVED — kept for context)* — Burrito Thursday ($15 House Burrito or Bowl) retired, replaced by **Big F’N Thursday** ($10 Big F’N Quesadilla + $10 Tequila Cocktails*). The live site was AHEAD of the repo; 27 repo files were behind. Thursday **poster/AI-image creative is still the retired burrito artwork** — new creative needed.
